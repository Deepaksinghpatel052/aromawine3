...
class AwProductsForm(forms.ModelForm):
    ...
    Color = forms.ModelChoiceField(required=True, empty_label="Please select Color",queryset=AwColor.objects.filter(Status=True),widget=forms.Select(attrs={"class": "form-control", 'name': 'Color'}))
    Bottel_Size = forms.ModelChoiceField(required=True, empty_label="Please select Bottel Size",
                                     queryset=AwSize.objects.filter(Status=True),widget=forms.Select(attrs={"class": "form-control", 'name': 'Bottel_Size'}))
    ...
    class Meta:
        model = AwProducts
        fields = ['Select_Type','LWineCode','AlcoholPercentage','Product_name','Producer','Category','Flavours','Color','Appellation','Bottel_Size','Classification','Vintage','Varietals','Country','Regions','Grape','Description','Meta_Title','Meta_Keyword','Meta_Description']