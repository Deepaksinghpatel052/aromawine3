...
class AwProductsForm(forms.ModelForm):
    ...
    Select_Type = forms.ModelChoiceField(required=True,  queryset=AwWineType.objects.filter(Status=True), widget=forms.Select(attrs={"class": "form-control" ,'name': 'Country'}))
    Producer = forms.ModelChoiceField(required=True,  queryset=AwProducers.objects.filter(Status=True), widget=forms.Select(attrs={"class": "form-control" ,'name': 'Country'}))
    Producer = forms.ModelChoiceField(required=True,  queryset=AwProducers.objects.filter(Status=True), widget=forms.Select(attrs={"class": "form-control" ,'name': 'Country'}))
    ...
    Regions = forms.ModelChoiceField(required=True,  queryset=AwRegion.objects.filter(Status=True), widget=forms.Select(attrs={"class": "form-control" ,'name': 'Country'}))
