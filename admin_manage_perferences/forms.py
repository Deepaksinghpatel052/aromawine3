...
class AwServiceInterestsForm(forms.ModelForm):
    ...
    Select_Type = forms.ModelChoiceField(required=True, empty_label="Please select type", queryset=AwInterestType.objects.filter(Status=True).order_by('Type'), widget=forms.Select(attrs={"class": "form-control" ,'name': 'interest_type'}))
